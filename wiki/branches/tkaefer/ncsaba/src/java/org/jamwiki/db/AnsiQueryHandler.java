...
public class AnsiQueryHandler implements QueryHandler {
    ...
    public void insertTopicVersion(TopicVersion topicVersion, Connection conn) throws Exception {
        ...
        int i = 1;
        stmt.setInt(i++, topicVersion.getTopicVersionId());
        stmt.setInt(i++, topicVersion.getTopicId());
        stmt.setString(i++, topicVersion.getEditComment());
        i = setTopic(stmt, i, topicVersion.getVersionContent());
        if (topicVersion.getAuthorId() != null) {
            stmt.setInt(i++, topicVersion.getAuthorId().intValue());
        } else {
            stmt.setNull(i++, Types.INTEGER);
        }
        stmt.setInt(i++, topicVersion.getEditType());
        stmt.setString(i++, topicVersion.getAuthorIpAddress());
        stmt.setTimestamp(i++, topicVersion.getEditDate());
        if (topicVersion.getPreviousTopicVersionId() != null) {
            stmt.setInt(i++, topicVersion.getPreviousTopicVersionId().intValue());
        } else {
            stmt.setNull(i++, Types.INTEGER);
        }
        stmt.executeUpdate(conn);
    ...
    private int setTopic(WikiPreparedStatement stmt, int i, String topic)
      throws Exception
    {
        if (Environment.getBooleanValue(Environment.PROP_TOPIC_COMPRESSION_ON)) {
            stmt.setNull(i++, Types.VARCHAR);
            stmt.setBytes(i++, IOUtil.zipContent(topic));
        } else {

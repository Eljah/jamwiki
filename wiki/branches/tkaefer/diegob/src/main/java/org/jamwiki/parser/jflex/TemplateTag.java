...
public class TemplateTag implements ParserTag {
	...
	// navigation values
	private static final String MAGIC_BACKLINK_LIST = "BACKLINKLIST";
	private static final String MAGIC_BACKLINK_LISTC = "BACKLINKLISTC";
	...
	static {
		...
		// navigation values
		MAGIC_WORDS.add(MAGIC_BACKLINK_LIST);
		MAGIC_WORDS.add(MAGIC_BACKLINK_LISTC);
	}
	...
	private String processMagicWord(ParserInput parserInput, String name) throws Exception {
		...
		*/
		// navigation values
		} else if (name.equals(MAGIC_BACKLINK_LIST) ||
				   name.equals(MAGIC_BACKLINK_LISTC)) {
			VirtualWiki vw = WikiBase.getDataHandler().lookupVirtualWiki(parserInput.getVirtualWiki());
			String stopAtTopic = null;
			if (vw != null) stopAtTopic = vw.getDefaultTopicName();
			return processBackLinks(topic.getName(), parserInput, "", stopAtTopic, name.equals(MAGIC_BACKLINK_LISTC)).trim();
		}
		return name;
	}

	private String processBackLinks(String topicName, ParserInput parserInput, String visitedTopics, String stopAtTopic, boolean complete) {
		String list = "[[" + topicName + "]]";
		String preTopic = "";
		
		// stop if already visited link is reached
		if ((("/" + visitedTopics + "/").indexOf("/" + topicName + "/") > 0))
			return "";

		visitedTopics += "/" + topicName;
		
		// only continue if stopAtTopic is not set or (if set) it is not reached
		if (stopAtTopic == null || (! topicName.equals(stopAtTopic))) {
			try {
				// get all predecessors of topicName
				Object[] results = getLinksTo(parserInput.getVirtualWiki(), topicName);
				String allPreLinks = "";
				int minLength = Integer.MAX_VALUE;
				for (int i = 0; i < results.length; i++) {
					preTopic = ((SearchResultEntry)results[i]).getTopic();
					// determine the trail StartingPoints ... -> preTopic -> topicName
					String preLinks = processBackLinks(preTopic, parserInput, visitedTopics, stopAtTopic, complete);
					if (preLinks.length() > 0) {
						if (! complete) {
							// no complete investigation, take the first trail which is found
							allPreLinks = preLinks;
							break;
						} else {
							// take the shortest trail which leads back to StartingPoints
							//logger.config("stopAtTopic: " + stopAtTopic + ", preLinks: " + preLinks);
							
							boolean leadsToStart = (stopAtTopic == null) || preLinks.startsWith("[[" + stopAtTopic + "]]");
// FIXME: counting depth by splitting is not elegant!							
							int depth = preLinks.split("&gt;").length;
							if ((depth < minLength) && leadsToStart) {
								minLength = depth;
								allPreLinks = preLinks;
							}
						}
				...
				if (allPreLinks.length() > 0)
					list =  allPreLinks + " &gt; " + list;

			} catch (Exception e) {
				return "(Exception: " + e + ") " + list;
			}
		}
		return list;
	}
	
	private Object[] getLinksTo(String virtualWiki, String topicName) throws Exception {

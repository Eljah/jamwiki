...
public class WikiSignatureTag {
	...
	private static final Logger logger = Logger.getLogger(WikiSignatureTag.class.getName());
			...
			if (includeUser) {
				...
				if (mode != JFlexParser.MODE_MINIMAL) {
					try {
						...
						logger.fatal("Failure while building wiki signature", e);
	...
	private String retrieveUserSignature(ParserInput parserInput) {
		...
		if (login == null || displayName == null) {
			logger.warn("Signature tagged parsed without user information available, returning empty");
			return "";
		}

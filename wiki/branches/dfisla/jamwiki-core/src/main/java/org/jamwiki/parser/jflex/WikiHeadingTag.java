...
public class WikiHeadingTag {
	...
	private String buildSectionEditLink(ParserInput parserInput, int section) {
		...
		if (parserInput.getLocale() == null) {
			logger.warn("Unable to build section edit links for " + parserInput.getTopicName() + " - locale is empty");
			return "";
		}
		...
		try {
			...
			logger.fatal("Failure while building link for topic " + parserInput.getVirtualWiki() + " / " + parserInput.getTopicName(), e);
		}

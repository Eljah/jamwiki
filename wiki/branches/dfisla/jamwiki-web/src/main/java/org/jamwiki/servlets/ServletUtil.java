    ...
    protected static String cachedContent(String context, Locale locale, String virtualWiki, String topicName, boolean cook) {
        ...
        try {
            if (cacheElement != null) {
                ...
                //ParsedTopic topicCacheObject = new ParsedTopic((String) cacheElement.getObjectValue());
                //content = topicCacheObject.getTopicContent();
    ...
    protected static void viewTopic(HttpServletRequest request, ModelAndView next, WikiPageInfo pageInfo, WikiMessage pageTitle, Topic topic, boolean sectionEdit, boolean allowRedirect) throws WikiException {
        ...
        try {
			...
			WikiUser wikiUser;
                ...
                if (topic.getTopicType() == Topic.TYPE_CATEGORY) {
                    next.addObject("categoryTopics", parsedTopic.categoryTopics);
                    next.addObject("numCategoryTopics", parsedTopic.categoryTopics.size());
                    next.addObject("categoryImages", parsedTopic.categoryImages);
                    next.addObject("numCategoryImages", parsedTopic.categoryImages.size());
                    next.addObject("subCategories", parsedTopic.subCategories);
                    next.addObject("numSubCategories", parsedTopic.subCategories.size());
                }

                if (topic.getTopicType() == Topic.TYPE_IMAGE || topic.getTopicType() == Topic.TYPE_FILE) {

                    next.addObject("fileVersions", parsedTopic.fileVersions);
